...
const useStyles = makeStyles((theme) => ({
  appBar: {
    position: "relative",
    background:
      "linear-gradient(90deg, rgba(48,158,185,1) 0%, rgba(48,102,152,1) 100%)",
  ...
  useEffect(() => {
    const getPokemon = _.debounce(async () => {
      ...
      const extraInfo = await Promise.all([
        fetch(`${URL}${pokemonInfo.name}-galar`)
          .then((response) => response.json())
          .catch((_) => {}),
        fetch(`${URL}${pokemonInfo.name}-alola`)
          .then((response) => response.json())
          .catch((_) => {}),
      ]);
        ...
        <Box padding={3}>
          ...
          <Autocomplete
            ...
            renderInput={(params) => (
              <TextField
                ...
                variant="outlined"
                placeholder="Selecciona los tipos para la tematica"
              />
            )}
          />
          ...
          <Autocomplete
            ...
            renderInput={(params) => (
              <TextField
                {...params}
                label="Pokemon baneados"
                variant="outlined"
                placeholder="Escribe el numero del pokemon o el nombre"
              />
            )}
          />
          <TextField
