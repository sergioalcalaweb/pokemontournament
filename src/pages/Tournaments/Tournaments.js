...
const Tournaments = ({ showNotification }) => {
  ...
  const userInTorurnament = (tID) =>
    userTournaments.find((t) => t.tournamentId === tID);
  ...
  const suscribe = async () => {
    try {
      if (user.pokemonNick) {
        ...
        showNotification(
          "Necesitas tener datos de pokemon go antes de inscribirte",
          "info"
        );
      }
    } catch (error) {
