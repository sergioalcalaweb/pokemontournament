...
const Positions = ({showNotification, participants, detail, matchs, createFinals, admin}) => {
  ...
  const handleCreate = async () => {
    if(selected.length === 10) {
      ...
      await createFinals(players);
      showNotification('Liguilla lista para empezar');
      setSelected([]);
      setSelectTrainees(false);
      setLoading(false);
    } else {
      showNotification('Necesitas 10 participantes para crear la liguilla', 'error');
    }
  }
