...
const DetailTopic = ({topic}) => {
  ...
  return (
    <Box>
      <Box display='flex' flexDirection={{ xs:"column", md:"row" }} justifyContent='center' width='100%' >        
        { topic.types.length > 0 && 
          <Box display='flex' flexDirection='column' alignItems='center' width={{ xs:"100%", md:"50%" }}>
            ...
            <Box display="flex"
              ...
              { topic.types.map( (type, index) => (                      
                <Chip
                  ...
                  avatar={<Avatar alt={type.name} src={`../types/${type.image}.png`}/>}                        
                />
              )) }
            </Box>            
          </Box>
      ...
      <Box display='flex' flexDirection={{ xs:"column", md:"row" }} mt={3} width="100%" >
        { topic.description && 
        ...
        { topic.code && 
          <Box display="flex" flexDirection="column"  width={{ xs:"100%", md:"40%" }}>            
            <Button
              variant="outlined" 
              size="small"  
