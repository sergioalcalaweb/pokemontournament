...
const useTrainee = () => {
  ...
  const userDetailsRef = firestore.collection("users").doc(userAuth.uid);

  const notificationsRef = firestore.collection("notifications");

  const { data: notifications } = useFirestoreCollectionData(
  ...
  const [info, setInfo] = useState({
    ...
    whatsapp: userFirebase.whatsapp,
    devices: userFirebase.devices || [],
  });

  const usersDetailsRef = useFirestore().collection("users");

  const { data: users } = useFirestoreCollectionData(usersDetailsRef, {
    idField: "id",
  });
  ...
  const deleteTorunament = async (id) => {
    const userTournament = await userTournaments
      .where("tournamentId", "==", id)
      .get();
    if (userTournament.docs.length > 0) {
      await Promise.all(
        userTournament.docs.map(
          async (tournament) => await tournament.ref.delete()
        )
      );
    }
  };
