...
const useTournaments = () => {
  ...
  const { data: all } = useFirestoreCollectionData(
    tournamentsRef.orderBy("timestamp", "desc"),
    { idField: "id" }
  );
  ...
  const createCupFromLeague = async (players, title, topic) => {
    ...
    const participants = players.map(
      ({ email, id, photoURL, pokemonID, pokemonNick, userID }) => ({
        email,
        id,
        photoURL,
        pokemonID,
    ...
    await Promise.all(
      participants.map(async (trainee, index) => {
  ...
  const close = async (id, kind) => {
    ...
    if (querySnapshot.docs.length > 0) {
      if (kind === "liga") {
        ...
        const { data: matchs } = generator(players, {
          type: "simple-cup",
          toBeDefinedValue: "En espera",
        });
  ...
  const addTrainee = async (trainee, type, id) => {
    ...
    const queryPlayers = await tournamentsRef
      .doc(id)
      .collection("participants")
      .get();
    await tournamentsRef
      .doc(id)
      .update({ participantsCount: queryPlayers.docs.length });
  };
  ...
  const addDemoParticipants = async (type, id, participants) => {
    ...
    await tournamentsRef
      .doc(id)
      .update({ participantsCount: participants + 7 });
  };
