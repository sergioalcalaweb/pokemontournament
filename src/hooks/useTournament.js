...
const useTournament = (id) => {
  ...
  const tournamentRef = firestore.collection("tournaments").doc(id);
  ...
  const { data: rules } = useFirestoreCollectionData(
    tournamentRef.collection("rules"),
    { idField: "id" }
  );
  const { data: topic } = useFirestoreDocData(
    tournamentRef.collection("rules").doc("topic")
  );
  ...
  const { data: matchs } = useFirestoreCollectionData(
    tournamentRef.collection("matchs").orderBy("day", "asc"),
    { idField: "id" }
  );
  const { data: games } = useFirestoreCollectionData(
    tournamentRef.collection("games").orderBy("order", "asc"),
    { idField: "id" }
  );
  const { data: repechages } = useFirestoreCollectionData(
    tournamentRef.collection("repechage").orderBy("order", "asc"),
    { idField: "id" }
  );
  const { data: finals } = useFirestoreCollectionData(
    tournamentRef.collection("finals").orderBy("order", "asc"),
    { idField: "id" }
  );
  ...
  const updateFinalMatch = async (info) => {
    if (info.type === "repechage") {
      ...
      repechage.position =
        info.homeResult > info.awayResult ? home.position : away.position;
  ...
  const deleteParticipant = async (trainee) => {
    ...
    const queryPlayers = await tournamentRef.collection("participants").get();
    await tournamentRef.update({ participantsCount: queryPlayers.docs.length });

    if (trainee.pokemonID !== "123412341234") {
      const userTournament = await firestore
        .collection("users")
        .doc(trainee.userID)
  ...
  const createFinals = async (participans) => {
    const finalPlayers = participans
      .slice()
      .map(({ userID, pokemonNick, photoURL }) => ({
        userID,
        pokemonNick,
        photoURL,
      }));
    const repechagePlayers = finalPlayers
      .splice(6, 4)
      .map((player, index) => ({ ...player, position: index + 7 }));
    const repechage1 = {
      title: "Repechaje",
      type: "repechage",
      order: 1,
      ...
      seeds: [
        {
          id: "SF1",
          teams: [
            { name: "En Espera", id: "CF1" },
            { name: "En Espera", id: "CF2" },
          ],
        },
        {
          id: "SF2",
          teams: [
            { name: "En Espera", id: "CF3" },
            { name: "En Espera", id: "CF4" },
          ],
        },
      ],
    };
      ...
      seeds: [
        {
          id: "F1",
          teams: [
            { name: "En Espera", id: "SF1" },
            { name: "En Espera", id: "SF2" },
          ],
        },
      ],
    };
  ...
  const addToFinals = () => {
    ...
    quarters.seeds[0].teams[1] =
      repechage1.position > repechage2.position
        ? repechage1User
        : repechage2User;
    quarters.seeds[2].teams[1] =
      repechage1.position > repechage2.position
        ? repechage2User
        : repechage1User;
  ...
  const nextStep = (winner, round, id, type) => {
    if (type === "finals") {
      if (finals.length === round + 1) {
        ...
        const seed = final.seeds.find(
          (seed) => seed.teams[0].id === id || seed.teams[1].id === id
        );
